...
export default function ProMainPage() {
  ...
  const fetchPosts = useCallback(async ({ queryKey, pageParam = 0 }: QueryFunctionContext<QueryKey>) => {
    ...
    const langQuery =
      selectedLanguages.length > 0 ? `&languages=${encodeURIComponent(JSON.stringify(selectedLanguages))}` : '';
  ...
  const { data, fetchNextPage, hasNextPage, isFetchingNextPage, isFetching } = useInfiniteQuery<Posts[], Error>({
